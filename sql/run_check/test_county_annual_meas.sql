WITH comparison as (
SELECT turnover, "in.state", meas, end_use,
...
FROM (
    SELECT 'commercial' AS source, turnover, end_use, county_ann_kwh AS kwh, "in.state", meas
    FROM long_county_annual_{turnover}_{weather}
    WHERE county_ann_kwh = county_ann_kwh AND "year" = {year} AND sector = 'com'

    UNION ALL
    
    SELECT 'residential' AS source, turnover, end_use, county_ann_kwh AS kwh, "in.state", meas
    FROM long_county_annual_{turnover}_{weather}
    WHERE county_ann_kwh = county_ann_kwh AND "year" = {year} AND sector = 'res'
    
    UNION ALL
    
    SELECT 'state_com' AS source, turnover, end_use, state_ann_kwh AS kwh, reg as "in.state", meas
    FROM scout_annual_state_{turnover}
    WHERE "year" = {year} AND fuel = 'Electric' AND sector = 'com'
    
    UNION ALL
    
    SELECT 'state_res' AS source, turnover, end_use, state_ann_kwh AS kwh, reg as "in.state", meas
    FROM scout_annual_state_{turnover}
    WHERE "year" = {year} AND fuel = 'Electric' AND sector = 'res'
) combined_results
GROUP BY turnover, end_use, "in.state", meas
)

SELECT turnover, end_use, "in.state", meas, commercial_sum, residential_sum, scout_commercial_sum, scout_residential_sum,
