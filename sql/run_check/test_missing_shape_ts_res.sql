...
-- Parameters: {year}, {turnover}, {weather}, {version}
...
good_shape_ts AS (
    SELECT shape_ts, "in.weather_file_city", "in.weather_file_longitude", end_use, sum(multiplier_hourly) as mult_sum
    FROM res_hourly_disaggregation_multipliers_{weather}
    WHERE multiplier_hourly = multiplier_hourly
    GROUP BY shape_ts, "in.weather_file_city", "in.weather_file_longitude", end_use
)
