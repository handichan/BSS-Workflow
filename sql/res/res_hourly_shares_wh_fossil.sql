...
WITH meta_shapes AS (
	...
	FROM "resstock_amy2018_release_2024.2_metadata" as meta
		RIGHT JOIN (SELECT * FROM res_ts_wh WHERE energy_source != 'electricity') as chars 
...
ts_not_agg AS (
	...
	meta_shapes."in.state",
		...
		ts."out.fuel_oil.hot_water.energy_consumption" + ts."out.natural_gas.hot_water.energy_consumption" + ts."out.propane.hot_water.energy_consumption" as wh
