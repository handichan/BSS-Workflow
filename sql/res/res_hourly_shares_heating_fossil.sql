...
WITH meta_shapes AS (
	...
	FROM "resstock_amy2018_release_2024.2_metadata" as meta
		RIGHT JOIN (SELECT * FROM res_ts_heating WHERE energy_source != 'electricity') as chars 
...
ts_not_agg AS (
	...
	meta_shapes."in.state",
		...
		ts."out.fuel_oil.heating.energy_consumption" + ts."out.natural_gas.heating.energy_consumption" + ts."out.propane.heating.energy_consumption" as heating
