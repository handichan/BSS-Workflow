...
-- aggregate to hourly by upgrade, weather file, and shape
ts_agg AS(
...
"in.weather_file_longitude",
	...
	timestamp_hour,
	dw as kwh,
	dw / sum(dw) OVER (PARTITION BY "in.state", "in.weather_file_city", "in.weather_file_longitude", shape_ts) as multiplier_hourly,
    'res' AS sector,
    "in.state",
	'Other' as end_use
FROM ts_agg
