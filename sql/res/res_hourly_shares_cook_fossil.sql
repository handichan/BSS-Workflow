...
WITH meta_shapes AS (
	...
	RIGHT JOIN (SELECT * FROM res_ts_cook WHERE energy_source != 'electricity') as chars 
...
ts_not_agg AS (
	...
	meta_shapes."in.state",
		...
		ts."out.electricity.natural_gas.energy_consumption" + ts."out.electricity.propane.energy_consumption" as cooking
