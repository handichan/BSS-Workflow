...
WITH meta_shapes AS (
	...
	RIGHT JOIN (SELECT * FROM res_ts_dry WHERE energy_source != 'electricity') as chars 
...
ts_not_agg AS (
	...
	meta_shapes."in.state",
		...
		ts."out.natural_gas.clothes_dryer.energy_consumption" + ts."out.propane.clothes_dryer.energy_consumption" as drying
