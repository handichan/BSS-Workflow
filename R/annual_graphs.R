...
wide<-data.frame()
for (file in list.files("scoutdata")){
  if (nrow(wide)==0) {
    wide<-bind_rows(wide,read_tsv(paste0("scoutdata/",file)))
  } else if(nrow(wide %>% filter(turnover=="baseline"))==0){
    wide<-bind_rows(wide,read_tsv(paste0("scoutdata/",file)))
  }else{
    wide<-bind_rows(wide,read_tsv(paste0("scoutdata/",file)) %>% filter(turnover!="baseline"))
  }
}
wide<-wide %>%
...
mm<-read_tsv("measure_map_20241021.tsv")
