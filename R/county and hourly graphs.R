...
setwd("/Users/mpigman/Library/CloudStorage/GoogleDrive-mpigman@lbl.gov/Shared drives/Buildings Policy Analysis Team/Projects/Buildings Standard Scenarios/Workflow test results/20240930/for R graphs")
...
county_ann_eu<-read_csv("county_ann_eu.csv") 
county_100_hrs<-read_csv("county_100_hrs.csv")
county_monthly_maxes<-read_csv("county_monthly_maxes.csv")
county_hourly_examples<-read_csv("county_hourly_examples.csv")
state_monthly_2024<-read_csv("state_monthly_2024.csv")
...
geo_counties<-read_csv("/Users/mpigman/Documents/DECARB/pipeline/mappings/geo/emm_county_map.csv") %>% filter(subregion!="yellowstone national") #we don't need 2 yellowstones
...
#sample sizes
ns<-arrow::read_parquet("ResStock2024_2baseline.parquet",col_select = c("bldg_id","in.county")) %>% group_by(in.county) %>% summarize(n=n())
...
plot_map_hist<-function(data_list){
  ...
  nice_labels <- setNames(
    purrr::map_chr(names(combined_plots), ~ {
      ...
      end_use_label <- eu[parts[2]] # Second part is end_use
      
      # Combine into a single label, skipping NA
      paste(na.omit(c(turnover_label, end_use_label)), collapse = " - ")
    }),
...
aggregated<-county_ann_eu %>%
  filter(sector=="com",end_use %in% c("Heating (Equip.)","Cooling (Equip.)","Ventilation"))
...
save_plot(paste0(filename,".jpg"),p,base_height = 12,bg="white")
...
save_plot("county_100_hrs_share.jpg",p100,base_height = 12,bg="white")
...
p100_hist<-county_100_hrs_share %>%
  ...
  theme(panel.grid.minor.y = element_blank(),axis.title = element_blank(),plot.margin=margin(b = 0),
        strip.background = element_blank(),strip.text.y = element_text(angle=-90,size=12),strip.text.x = element_text(size=12))+
  ggtitle("Share of Top 100 Hours in the Winter")
save_plot("county_100_hrs_share_hist.jpg",p100_hist,base_height = 6,base_width = 12,bg="white")
...
save_plot("county_ratio.jpg",ratio_map,base_height = 12,bg="white")
...
ratio_hist<-peak_ratio %>%
  ...
  theme(panel.grid.minor.y = element_blank(),axis.title = element_blank(),plot.margin=margin(b = 0),
        strip.background = element_blank(),strip.text.y = element_text(angle=-90,size=12),strip.text.x = element_text(size=12))+
  ggtitle("Ratio of Winter Peak to Summer Peak")
save_plot("county_ratio_hist.jpg",ratio_hist,base_height = 6,base_width = 12,bg="white")
...
# county_names<-meta_com %>% select(in.nhgis_county_gisjoin,in.county_name) %>% unique()
example_counties<-county_hourly_examples %>% select(in.county,example_type) %>% unique() %>%
  left_join(county_names,by=c("in.county"="in.nhgis_county_gisjoin")) %>%
  mutate(example_type=case_when(example_type=="hot" ~ "Hot",
         ...
         label=paste0(example_type,": ",in.county_name))
...
save_plot("example_peak_days.jpg",psample,base_height = 6,base_width = 12,bg="white")
...
save_plot("eia_seasonal_ratio_comp.jpg",eia_comp,base_height = 6,base_width = 12,bg="white")
