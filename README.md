...
`(C) Ref. Case Data Centers` is an exception to this exception. Because ComStock does not model standalone data centers, roughly 70-78% of this measure's electricity falls into the ComStock gap, a far larger share than other measures, and the real gap load shape (`com_gap_ts_1`) has large diurnal swings (peaking overnight and troughing at midday, the inverse of a typical commercial profile) that projecting it onto a measure this influential to the commercial forecast risked distorting the overall shape. There is also no public data to support a specific data center load shape, so we assign a flat load shape (`com_flat_ts`, `multiplier_hourly = 1/8760`) instead of `com_gap_ts_1`, while keeping the ComStock gap model's geographic disaggregation (`com_gap_ann_1`) for this measure. We route its gap-model portion to its own `measure_map.tsv` entry (`Gap - Data Centers`, via `Config.GAP_MEAS_OVERRIDE` in `bss_workflow.py`) rather than the shared `Gap` measure, so this override doesn't change the treatment of every other measure's gap portion.
