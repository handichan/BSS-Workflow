...
`(C) Ref. Case Data Centers` is an exception to this exception. Roughly 70-78% of its electricity falls into the ComStock gap, a far larger share than other measures, and the real gap load shape (`com_gap_ts_1`) is volatile enough (double-peaked) that projecting it onto a measure this influential to the commercial forecast risked distorting the overall shape. There is also no public data to support a specific data center load shape. We therefore keep the ComStock gap model's geographic disaggregation (`com_gap_ann_1`) for this measure but assign a flat load shape (`com_flat_ts`, `multiplier_hourly = 1/8760`) instead of `com_gap_ts_1`, and route its gap-model portion to its own `measure_map.tsv` entry (`Gap - Data Centers`, via `Config.GAP_MEAS_OVERRIDE` in `bss_workflow.py`) rather than the shared `Gap` measure, so this override doesn't change the treatment of every other measure's gap portion.
